...
class Net(nn.Module):
    def __init__(self):
        ...
        self.tran_embed_dim = C.RPIN.TRAN_EMBED_DIM
        ...
        self.pool_size = C.RPIN.ROI_POOL_SIZE
        ...
        self.embedding = nn.Linear(self.in_feat_dim * self.pool_size * self.pool_size, self.tran_embed_dim)

        self.transformer_encoder_layer = nn.TransformerEncoderLayer(
            d_model=self.tran_embed_dim,
            batch_first=True,
            nhead=C.RPIN.TRAN_HEADS_NUM,
        ...
        self.bbox_decoder = nn.Linear(self.in_feat_dim * self.pool_size * self.pool_size, self.decoder_output)

        if C.RPIN.MASK_LOSS_WEIGHT > 0:
            self.mask_decoder = nn.Sequential(
                nn.Linear(self.in_feat_dim * self.pool_size * self.pool_size, self.in_feat_dim),
                nn.ReLU(inplace=True),
        ...
        if C.RPIN.SEQ_CLS_LOSS_WEIGHT > 0:
            self.seq_feature = nn.Sequential(
                nn.Linear(self.in_feat_dim * self.pool_size * self.pool_size, self.in_feat_dim * 4),
                nn.ReLU(inplace=True),
    ...
    def forward(self, x, rois, num_rollouts=10, g_idx=None, x_t=None, phase='train'):
        ...
        for i in range(num_rollouts):
            c = [self.transformer(state_list[j].reshape(list(state_list[j].shape[:2])+[-1])) for j in range(self.time_step)]
            c = [c_.reshape(list(c_.shape[:2])+[self.in_feat_dim, self.pool_size, self.pool_size]) for c_ in c]
            all_c = torch.cat(c, 2)
